...
In order to define the data that will be added or removed from the database one or more [fixtures]() must be created.  Note that the fixture file names must follow the configuration you set for [Crave](https://github.com/ssmereka/crave/blob/master/readme.md#config) in [Cramit's configuration object]().  By default, Crave looks for and requires any file that contains "_fixture" in the name.
